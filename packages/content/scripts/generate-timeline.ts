...
export async function processAboutFile(): Promise<AboutData> {
  try {
    const content = await readFile(
      path.join(PACKAGE_ROOT, "content/about.md"),
      "utf-8",
    );
    const parsed = matter(content);
    ...
    console.error(
      `\x1b[31m[About] Error processing about.md: ${message}\x1b[0m`,
    );
    throw err;
  }
}
  ...
  try {
    ...
    for (const filepath of files) {
      try {
        ...
        const message = err instanceof Error ? err.message : String(err);
        console.error(
          `\x1b[31m[Timeline] Error in ${filepath}: ${message}\x1b[0m`,
        );
        process.exit(1);
      }
    }
    ...
    const filteredNodes = nodes.filter(
      (n) => new Date(n.date).getTime() >= JAN_2017,
    );
    ...
    await mkdir(path.join(REPO_ROOT, "apps/portfolio/public"), {
      recursive: true,
    });
    ...
    console.log(
      `[Timeline] ✓ Generated timeline.json with ${nodes.length} entries`,
    );
