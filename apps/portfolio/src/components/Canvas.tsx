...
export function Canvas() {
  ...
  useEffect(() => {
    if (!isGameMode || !stageRef.current) {
          ...
          <Layer>
                ...
                return yearPositions.map(({ year, x }) => {
                  const opacity = Math.max(
                    ...
                    Math.min(
                      1,
                      1 -
                        (zoom - YEAR_LABEL_ZOOM_START) /
                          (YEAR_LABEL_ZOOM_END - YEAR_LABEL_ZOOM_START),
                    ),
                  );
            ...
            {positionedNodes.map(({ node, x, y }) => {
              ...
              const labelOpacity = Math.max(
                ...
                Math.min(
                  1,
                  (zoom - DATE_LABEL_ZOOM_START) /
                    (DATE_LABEL_ZOOM_END - DATE_LABEL_ZOOM_START),
                ),
              );
